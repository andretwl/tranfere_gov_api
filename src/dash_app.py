...
def build_layout():
    children = [
        html.Header(style={"marginBottom": "2rem", "borderBottom": "1px solid #334155", "paddingBottom": "1rem"}, children=[
            html.H1("TransfereGov Intel — Dash & MCP Analytics Hub", style={"color": "#f8fafc", "fontSize": "2rem", "marginBottom": "0.5rem"}),
            html.P("Painel analítico automatizado via Plotly Express e servidor MCP (Model Context Protocol).", style={"color": "#94a3b8", "fontSize": "1rem"}),
            html.Div(style={"display": "inline-block", "marginTop": "0.5rem", "padding": "0.3rem 0.8rem", "background": "#1e293b", "borderRadius": "20px", "border": "1px solid #3b82f6", "color": "#60a5fa", "fontSize": "0.85rem"}, children=[
                "⚡ Servidor MCP ativo no endpoint: ",
                html.Code("http://localhost:8050/_mcp", style={"color": "#38bdf8", "fontWeight": "bold"})
            ])
        ])
    ]
    ...
    for chart_id, spec in CHART_REGISTRY.items():
        ...
        for ctrl in spec.controls:
            ...
            controls_html.append(
                html.Div(style={"marginRight": "1.5rem", "marginBottom": "1rem"}, children=[
                    html.Label(ctrl.label, style={"display": "block", "fontSize": "0.85rem", "color": "#94a3b8", "marginBottom": "0.3rem"}),
                    dcc.Dropdown(
        ...
        card = html.Div(style={"backgroundColor": "#1e293b", "borderRadius": "12px", "padding": "1.5rem", "marginBottom": "2rem", "border": "1px solid #334155"}, children=[
            html.Div(style={"display": "flex", "justifyContent": "space-between", "alignItems": "flex-start"}, children=[
                html.Div([
                    html.H2(spec.title, style={"color": "#f8fafc", "fontSize": "1.25rem", "marginBottom": "0.3rem"}),
                    html.P(spec.description, style={"color": "#94a3b8", "fontSize": "0.85rem", "marginBottom": "1rem"})
                ]),
                html.Span(spec.category, style={"padding": "0.2rem 0.6rem", "borderRadius": "4px", "background": "#334155", "color": "#cbd5e1", "fontSize": "0.75rem"})
            ]),
            html.Div(style={"display": "flex", "flexWrap": "wrap"}, children=controls_html) if controls_html else html.Div(),
            dcc.Graph(id=f"graph-{chart_id}", figure=initial_figure)
        ])
        children.append(card)

    return html.Div(style={"backgroundColor": "#0f172a", "minHeight": "100vh", "padding": "2rem", "fontFamily": "Inter, sans-serif"}, children=children)
