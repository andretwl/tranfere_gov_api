...
def verify_single_chart(chart_id: str, spec: Any) -> tuple[bool, int, int, float, str]:
    ...
    try:
        ...
        for trace in fig.data:
            ...
            link_len = (
                len(trace.link.value)
                if hasattr(trace, "link")
                and trace.link
                and hasattr(trace.link, "value")
                and trace.link.value is not None
                else 0
            )
...
def run_verification_suite() -> bool:
    ...
    for chart_id, spec in CHART_REGISTRY.items():
        ...
        print(
            f"{status_str:<12} | {chart_id:<35} | {num_traces:<7} | {total_pts:<8} | {elapsed_ms:>6.1f}ms"
        )
    ...
    print("=" * 80)
    print(
        f"📊 RESUMO DE EXECUÇÃO: {passed_count}/{total_charts} Gráficos Operacionais ({passed_count / total_charts * 100:.1f}%)"
    )
    print("=" * 80)
