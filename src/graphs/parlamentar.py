...
def chart_eficiencia_partidos(uf_filter: str = "TODOS") -> go.Figure:
    ...
    fig = px.bar(
        df, x="sigla_partido", y="valor_total", color="status_execucao",
        title="1. Eficiência na Execução de Emendas por Partido",
...
def chart_top_parlamentares() -> go.Figure:
    ...
    fig = px.bar(
        df, x="valor_total", y="parlamentar_nome", orientation="h",
        text_auto=".2s", color="valor_total", color_continuous_scale="Viridis",
...
def chart_impedimentos_por_partido() -> go.Figure:
    ...
    fig = px.scatter(
        df, x="valor_total", y="taxa_impedimento_pct", size="total_planos",
        color="taxa_impedimento_pct", text="partido", color_continuous_scale="OrRd",
