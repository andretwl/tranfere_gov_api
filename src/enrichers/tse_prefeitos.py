...
async def sync_prefeitos(
    dry_run: bool = False, target_uf: str | None = None, ano: int = 2024
) -> None:
    conn = get_connection()
    cur = conn.cursor()
    ...
    print(
        f"📊 Prefeitos eleitos retornados: {len(tse_rows)} | Vice-prefeitos eleitos: {len(tse_vices)}"
    )
    ...
    print(
        f"✅ Prefeitos mapeados com sucesso: {len(matched_records)} | Sem match: {unmatched_count}"
    )
